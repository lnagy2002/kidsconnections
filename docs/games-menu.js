...
(() => {
  ...
  .gm-panel{position:fixed;top:0;right:0;height:100dvh;width:min(320px,86vw);background:#0e364b;color:#a9f4f4;border-left:1px solid #103954;transform:translateX(100%);transition:transform .25s ease;z-index:9999;display:flex;flex-direction:column}
  ...
  function injectDOM() {
    if (document.getElementById('gm-burger')) return;
    ...
    document.body.appendChild(burger);document.body.appendChild(overlay);document.body.appendChild(panel);
  }

  function fillList(games){
    const list=document.getElementById('gm-list');if(!list)return;
    list.innerHTML='';const current=byPath(location.href);
    games.forEach(g=>{
  ...
  function wireBehavior(){
    ...
    const open=()=>{
      ...
      panel.classList.add('gm-open');overlay.classList.add('gm-open');
      panel.setAttribute('aria-hidden','false');
      const firstLink=panel.querySelector('a');(firstLink??panel).focus({preventScroll:true});
      document.addEventListener('keydown',onKey);
    };
    const close=()=>{
      burger.setAttribute('aria-expanded','false');
      panel.classList.remove('gm-open');overlay.classList.remove('gm-open');
    ...
    function onKey(e){
      ...
      if(e.key==='Tab'&&panel.classList.contains('gm-open')){
        const foci=panel.querySelectorAll('a,button,[tabindex]:not([tabindex="-1"])');if(!foci.length)return;
  ...
  async function loadConfig(){
    ...
    if(Array.isArray(window.GAMES_MENU)&&window.GAMES_MENU.length)return window.GAMES_MENU;
    const url=attrUrl||'/games-menu.json';
    try{
      const res=await fetch(url,{credentials:'same-origin'});if(!res.ok)throw 0;
